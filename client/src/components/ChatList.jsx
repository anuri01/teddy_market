...
function ChatList({ onClose, onRoomSelect }) {
    ...
    const handleDeleteRoom = async (e, roomIdToDelete) => {
        e.stopPropagation(); // 이벤트 버블링 방지(항상 주의)
        if(window.confirm('채팅방을 삭제하면 대화내용이 사라집니다.')) {
            try {
               await api.delete(`/chat/rooms/${roomIdToDelete}`);
               toast.success('대화방이 삭제됐습니다.')
               //ui에서 바로 삭제
               setRooms(prevRooms => prevRooms.filter(room => room._id !== roomIdToDelete));
        } catch(error) {
                    ...
                    rooms.length > 0 ? (
                        rooms.map(room => (
                            <div key={room._id} className="chat-room-item" onClick={() => onRoomSelect(room)}>
                                ...
                                <button type="button" className='chat-action-button button-primary' onClick={(e) => handleDeleteRoom(e, room._id)}>삭제</button>
                            </div>
                        ))
                    ) : (
